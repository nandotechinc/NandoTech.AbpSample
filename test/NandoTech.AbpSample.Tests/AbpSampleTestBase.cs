...
using NandoTech.AbpSample.Migrations.SeedData;
...
namespace NandoTech.AbpSample.Tests
{
    public abstract class AbpSampleTestBase : AbpIntegratedTestBase<AbpSampleTestModule>
    {
        ...
        protected void UsingDbContext(Action<AbpSampleDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<AbpSampleDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<AbpSampleDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<AbpSampleDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<AbpSampleDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AbpSampleDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<AbpSampleDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AbpSampleDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<AbpSampleDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<AbpSampleDbContext, Task<T>> func)
        {
            T result;
